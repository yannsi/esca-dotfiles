...
# 既に起動している場合は終了（トグル動作）
pid_file = "/tmp/waybar_volume_slider.pid"
...
def set_volume(val):
    try:
        val_float = val / 100.0
        subprocess.run(["wpctl", "set-volume", "@DEFAULT_AUDIO_SINK@", f"{val_float:.2f}"])
    except Exception:
        pass
...
class VolumePopup(Gtk.Window):
    def __init__(self):
        ...
        GtkLayerShell.set_margin(self, GtkLayerShell.Edge.RIGHT, 130)
        GtkLayerShell.set_keyboard_mode(self, GtkLayerShell.KeyboardMode.ON_DEMAND)

        self.set_title("Volume Slider")
        ...
        # Icon
        self.icon_label = Gtk.Label(label="󰕾")
        box.pack_start(self.icon_label, False, False, 0)

        # Slider
        cur_vol = get_volume()
        self.adjustment = Gtk.Adjustment(value=cur_vol, lower=0, upper=100, step_increment=1, page_increment=5)
